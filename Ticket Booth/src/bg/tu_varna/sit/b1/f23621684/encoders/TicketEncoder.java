...
public class TicketEncoder {
    ...
    public static SeatInfo decode(String encoded, Event event) {
        ...
        if (event == null)
            event = EventDataReporter.getEvent(parts[0], date);
        if (event == null)
            throw new EventNotFound("Could not find event " + parts[0] + " on date " + date);
        if(!event.getName().equals(parts[0]) || !event.getDate().equals(date))
            throw new InvalidCodeException("Ticket information in code does not match the event information.");
